...
@csrf_exempt
def frl_optimised_ovr_view_dash(request):
    if request.method == GET:
        try:
            ...
            final_result = []
            if param == '0':
                data = django_search_query(RUN_OPTIMISED_DATA.format(run=run))
                data = data[0]['json_build_object']
                final_result.append(data)
            else:
                run = django_search_query(OPTIMISED_MAX_RUN)
                data = django_search_query(RUN_OPTIMISED_DATA.format(run=run[0]['max']))
                data = data[0]['json_build_object']
                final_result.append(data)
